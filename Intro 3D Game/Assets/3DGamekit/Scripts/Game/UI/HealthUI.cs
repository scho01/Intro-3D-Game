...
namespace Gamekit3D
{
    public class HealthUI : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            m_HealthIconAnimators = new Animator[Mathf.RoundToInt((float)representedDamageable.maxHitPoints)];

            for (int i = 0; i < Mathf.RoundToInt((float)representedDamageable.maxHitPoints); i++)
            {
                ...
                if (Mathf.RoundToInt((float)representedDamageable.currentHitPoints) < i + 1)
                {
        ...
        public void ChangeHitPointUI(Damageable damageable)
        {
            ...
            {
                m_HealthIconAnimators[i].SetBool(m_HashActivePara, Mathf.RoundToInt((float)damageable.currentHitPoints) >= i + 1);
            }
        }
    } 
}
