...
namespace Gamekit3D
{
    ...
    public class PlayerController : MonoBehaviour, IMessageReceiver
    {
        ...
        public float healthDecaySpeedModifier = 0.005f;
        ...
        {
            if (!m_Damageable.isInvulnerable)
            {
                double decay = Time.deltaTime * healthDecayRate;
                decay += m_CharCtrl.velocity.magnitude * healthDecaySpeedModifier;
                m_Damageable.Decay(decay);
                if (m_Damageable.currentHitPoints <= 0)
                    DecayDie();
            }
        }

        public void DecayDie()
        {
            m_Animator.SetTrigger(m_HashDeath);
            m_ForwardSpeed = 0f;
            m_VerticalSpeed = 0f;
        ...
        public void Heal(bool apple)
        {
            m_Damageable.HealDamage(m_Food.Consume(apple));
        }
    }
}